...
void setup() {
  ...
  // add pinMode assertions for safeboot and rst.
  ...
  // Begin CY. If address is incorrect, note it and stop running.
  if (!cy.begin()) {
    DBG_SERIAL.println("Incorrect CY8C95X0");
    set_status(FAILED);
    while(1);
  }
...
void loop() {
  ...
  // Move the code into this loop. Returning won't do anything.
  if (digitalRead(PIN_BTN_TEST) == LOW) {
    set_status(BUSY);

    // Test harness by checking the connectivity of every pin
    // Invert logic. Start passing, set false for every failure. This ensures a failure is properly caught.
    bool passed = true;
    ...
    // Add delay so the operator can actually see the LED.
    delay(1000);
